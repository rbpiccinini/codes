...
Ly = 1.
Lz = 1.
mesh = fd.utility_meshes.RectangleMesh(nx=30, ny=30, Lx=Lx, Ly=Ly,
                                       quadrilateral=True)
nz = 10
...
W = fd.TensorFunctionSpace(mesh, "DG", 0)
...
Delta_h = fd.sqrt(fd.jump(x_func)**2 + fd.jump(y_func)**2)
...
a = fd.jump(u)/Delta_h*fd.jump(v)*fd.dS
m = u * v * dx
