...
class classPVT:
	...
	def visco(self,p):
		return interp(p,self.pe[::-1],self.viscoe[::-1])

	def viscw(self,p):
		return interp(p,self.pe[::-1],self.viscwe[::-1])

	def cw(self,p,pref):
		return -(self.Bw(p)-self.Bw(pref))/(p-pref+0.01)/self.Bw(pref)
...
class classAquifero:
	...
	def We(self,p,dt=0,Np=0):
		if self.model == 'tank':
			...
			return self.FetPcte(p,dt)
		elif self.model == 'user':
			return interp(p,self.pee,self.Wee)
 		elif self.model == 'user_Np':
			return interp(Np,self.Npe,self.Wee)
		elif self.model == 'None':
			return 0.

	def tank(self,p):
		ct=self.cr+self.cw
		r=ct*self.WW*(self.p0-p)
		return r

	def FetPcte(self,p,dt):
	#   Fetkovitch model for linear flux
	#   "t" is given in days
		self.time=self.time+1
		J=self.k*self.WW/self.pvt.viscw(p)/self.L**2/39.85 # Sistema Petrobras
		ct=self.cr+self.cw
		Wei=ct*self.WW*(self.p0-p)
		return Wei*(1.-p/self.p0)*(1.-exp(-J*self.p0*dt/Wei))

	def pa(self,We):
		ct=self.cr+self.cw
		return self.p0*(1.-We/(ct*self.WW*self.p0))
...
class classPetrophys:
	krwe=0.
	kroe=0.
	Swe=0.

	def kro(self,Sw):
		return interp(Sw,self.Swe,self.kroe)
	def krw(self,Sw):
		return interp(Sw,self.Swe,self.krwe)
