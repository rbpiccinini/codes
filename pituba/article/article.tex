...
Derivation of the material balance equation consists in assuming no fluid motion and in integrating black-oil equations in reservoir volume. The integration process, however, requires all quantities to be either spatially averaged or assumed constant.
...
We now define spatially averaged quantities for porosity ($\phi$), saturation ($S_\alpha$), density ($\rho_\alpha$) and mass concentration of a component $k$ in phase $\alpha$ ($Y_{\alpha,k}$).
...
Material balance equation is widely employed to stablish a relation between production (and injection) and reservoir pressure. It turns out, as we know, that pressure is different for different locations, mostly for different values of depth, and also for different phases.

What is then the significance of a single value of pressure for the entire reservoir? 

We shall see that for the purpose of computing the averaged value of phase densities in material balance, we may define some reference depth and use the corresponding local pressure without introducing any important error. Phase pressure increases with depth according to the hydrostatic gradient. The pressure increase, however, is not significant to produce appreciable variation in density.

%and it may be approximated to a constant density measured at some reference depth.

We may quantify this approximation by putting together the expressions for compressibility and hydrostatic equilibrium. For this, we assume a constant isothermal compressibility for oil phase
