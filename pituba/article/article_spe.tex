...
\end{equation}
where $p_\alpha (t,z)$ is the pressure field of phase $\alpha$ and $p_\alpha^{ref} (t)$ is the pressure at the datum level $z_{ref}$.
...
The datum level for computing phase density (and also the formation volume factor) can be any arbitrary depth inside phase volume. Under the approximation of a constant density, it is straightforward to show that $\bar{\rho}_\alpha = \rho_\alpha\left(\bar{p}_\alpha\right)$ and $\bar{p}_\alpha=p_\alpha\left(z^0_\alpha\right)$, where $z^0_\alpha$ is the depth of phase gravity center for $\alpha=w, o, g$. The pressure calculated in the material balance equation corresponds to the local pressure at $z^{ref}$ for negligible hydrodynamic gradients and only equals the average reservoir pressure $\bar{p}_\alpha$ if the datum level is that of the gravity center, $z^{ref}=z^0$.

For the case of an oil reservoir with a gas cap, the datum level can be defined at the gas-oil contact. The point here is that at gas-oil contact both gas and oil densities are computed using the pressure local value. If the datum level, common to all phases, is otherwise defined below gas-oil contact, a higher gas density will be computed because of the oil hydrostatic gradient:
\begin{equation}
