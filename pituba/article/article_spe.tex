...
\frac{\partial}{\partial t} \int\limits_{\Omega} \rho_w S_w \phi dV = - \oint_{\partial \Omega} \rho_w \left( \vec{u}_w \cdot \vec{dS} \right)= \rho_w^{ST}\left(Q_{we}^{ST} + Q_{wi}^{ST}-Q_w^{ST}\right) \, ;
\end{equation}
...
It is now possible to obtain the general material balance equation substituting the equations for phase saturation, Eqs.~\eqref{eq: Sw5}, \eqref{eq: So5} and \eqref{eq: Sg5}, into Eq.~\eqref{eq: S15}:  
...
\subsubsection{An interpretation for the calculated pressure}
...
A possible simplification to the pressure field is neglecting hydrodynamic gradients and assuming hydrostatic equilibrium:
...
\end{equation}
This assumption implies the identity of pressure gradient and gravity force per unit volume
\begin{equation}\label{eq: p=rhogh}
\nabla p_\alpha \equiv \rho_\alpha \vec{g} \, ,
\end{equation}
and, consequently, pressure becomes a function of time and depth only:
\begin{equation}\label{eq: p=rhogh}
\int_{p_\alpha^0 (t)}^{p_\alpha} \frac{dp_\alpha^{'}}{\rho_\alpha \left(p_\alpha^{'}\right)} = g\left(z - z_0\right) \, ,
\end{equation}
where $p_\alpha^0 (t)$ is the pressure of phase $\alpha$ at some reference depth $z_0$ and time $t$.
...
Eq.~\eqref{eq: PVTH1} also provides one condition to check whether PVT data obtained from laboratory or empirical correlations is physically correct. It can be alternatively formulated as shown in Eq.~\eqref{eq: vaporization} and it simply indicates that any vaporization must be followed by an increase in oil phase density:
...
$m$ & ratio of original free gas volume to original oil phase volume in reservoir condition \\
