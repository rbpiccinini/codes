...
\end{equation}
where $p_\alpha (t,z)$ is the pressure field of phase $\alpha$ and $p_\alpha (t,z_{datum})$ is the pressure at the datum level $z_{datum}$.
It is now possible to compute the pressure field from Eq.~\eqref{eq: p=rhogh} and average fluid properties accordingly.

It can be shown that neglecting the variation of density with depth does not introduce  important errors. As pressure increases with depth according to the hydrostatic gradient, density also increases but by a negligible amount. For instance, the average oil density can be approximated to the local density at the datum level: $\bar{\rho}_o (t) \approx \rho_o (t, z_{datum})$.
...
The datum level for computing phase density (or the formation volume factor) can be any arbitrary depth inside phase volume. The pressure calculated in the material balance equation corresponds to the local pressure at $z_{datum}$ for negligible hydrodynamic gradients.
