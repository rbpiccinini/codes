...
\begin{abstract}
This work derives the classical material balance equation from the more general black-oil model. The material balance equation is formulated for a varying bubble-point pressure and sufficient conditions for solution uniqueness are presented. It is shown that spatially averaged PVT properties are required for material balance approach in general, though the averaging process may be neglected for many practical purposes.  An interpretation for pressure in material balance equation is provided and its relation to a reference depth is examined. Finally, a scheme for numerical solution is proposed.

Material balance equation and black-oil models are approaches widely employed in reservoir engineering though few works explore their connection beyond the general intuitive notion. 
%
...
Momentum balance for each phase is given by an extension of Darcy's law to multiphase flow. This extension consists in multiplying absolute (or single-phase) permeability ($\overleftrightarrow{k}$) by a scalar function called phase relative permeability ($k_{r,\alpha}$):

\begin{equation}\label{eq: exdarcy}
\vec{u}_\alpha = -\frac{k_{r,\alpha} \vec{k}}{\mu_\alpha} \left(\nabla p_\alpha - \rho_\alpha g\nabla z\right) \, , \quad \alpha=o,w,g \, .
\end{equation}
...
p_{c,ow}\left(\vec{k},\phi,S_w\right)=p_o-p_w \, ,
\end{equation}
\begin{equation}\label{eq: capillary_og}
p_{c,go}\left(\vec{k},\phi,S_g\right)=p_g-p_o \, .
\end{equation}
...
%\begin{equation}
%\frac{\partial}{\partial t}\left(\frac{\phi (p_P) S_w}{B_w (p_w)}\right)-\nabla\cdot\left[\frac{k_{r,w} \vec{k}}{B_w(p_w)\mu_w} \left(\nabla p_o - \rho_w g\nabla z -\underbrace{\frac{d p_c}{d S_w}\nabla S_w}_{\text{water diffusion}}\right)\right]=q_{ws} \, .
%\end{equation}
%
%Discontinuities in saturation may still occur, however, if capillary pressures are functions of discontinuous porous medium properties ($phi$ and $\vec{k}$) or if phase change takes place, as diffusion of gas dissolved in oil phase is not allowed.
...
Material balance equation is widely employed to stablish a relationship between production (and injection) and reservoir pressure. It turns out, though, that pressure is different for different locations, mostly for different depths, and also for different phases.
