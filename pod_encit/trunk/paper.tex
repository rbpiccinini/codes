...
\abstract{The proper orthogonal decomposition (POD) is applied to particle image velocimetry (PIV) datasets of an engine static flow bench. Different flow fields are produced changing intake port design and flow snapshots are taken at a fixed position downstream port exit. It is observed that a swirl motion is mostly contained in the first POD mode and that the swirl motion steadiness depends on the amount of kinetic energy in the first mode. All higher POD modes showed pairs of counter rotating vortices with zero mean angular momentum.
}
