...
def PODmodes(outputdir,malha,B,field,v,w,fr):
	...
	while e[i]<fr:
		...
		ui=griddata(x,y,POD[i].uvw[:,0],xi,yi)
		vi=griddata(x,y,POD[i].uvw[:,1],xi,yi)
		wi=griddata(x,y,POD[i].uvw[:,2],xi,yi)
		...
		for n in range(len(xi)):
			for m in range(len(xi)):
				if sqrt(xi[m,n]**2+yi[m,n]**2) >= B/2:
					ui[m,n],vi[m,n],wi[m,n]=0.,0.,0.
				
				
				pts=pts+[xi[m,n],yi[m,n],0]
				vec=vec+[float(ui[m,n]),float(vi[m,n]),float(wi[m,n])]
...
#inputdir='/home/piccinini/work/POD/python/sygma/snaps/'
#outputdir='/home/piccinini/work/POD/python/sygma/pods/'
#main(0.128,100,inputdir,outputdir)

#inputdir='/home/piccinini/work/POD/python/scania/snaps/'
#outputdir='/home/piccinini/work/POD/python/scania/pods/'
