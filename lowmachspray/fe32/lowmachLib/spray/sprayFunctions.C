...
scalar spray::minD() const
{
    ...
    {
        minD = min(minD, elmnt().d());
    }

    reduce(minD, minOp<scalar>());

    return minD;
}

scalar spray::maxTd() const
{
    scalar maxTd = 0.0;
    ...
    {
        maxTd = max(maxTd, elmnt().T());
    }

    reduce(maxTd, maxOp<scalar>());

    return maxTd;
}

scalar spray::minTd() const
{
    scalar minTd = 1e6;
    ...
    {
        minTd = min(minTd, elmnt().T());
    }

    reduce(minTd, minOp<scalar>());
