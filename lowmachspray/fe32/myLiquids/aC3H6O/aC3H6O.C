...
namespace Foam
{
    defineTypeNameAndDebug(aC3H6O, 0);
    addToRunTimeSelectionTable(liquid, aC3H6O,);
    addToRunTimeSelectionTable(liquid, aC3H6O, Istream);
}
...
(
    const liquid& l,
    const rhoFunc& density,
    const pvFunc& vapourPressure,
    const NSRDSfunc6& heatOfVapourisation,
    const cpFunc& heatCapacity,
    const NSRDSfunc0& enthalpy,
    const NSRDSfunc7& idealGasHeatCapacity,
    const NSRDSfunc4& secondVirialCoeff,
    const muFunc& dynamicViscosity,
    const NSRDSfunc2& vapourDynamicViscosity,
    const KFunc& thermalConductivity,
    const NSRDSfunc2& vapourThermalConductivity,
    const NSRDSfunc0& surfaceTension,
    const DFunc& vapourDiffussivity
)
:
    liquid(l),
    rho_(density),
